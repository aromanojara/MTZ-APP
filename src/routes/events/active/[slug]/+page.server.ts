...
export const actions = {
	...
	LeaveClassWaitList: async (event, params) => {
		...
		// const quota = parseInt(dataEvents[0].quota) + 1;
		// const attendance = parseInt(dataEvents[0].attendance) - 1;	
...
export const load: PageServerLoad = async function({ params, cookies, locals }) {
	...
	dataEvents[0].fecha = dataEvents[0].date.toLocaleString("es-CL", {timeZone: 'America/Santiago'}).split(',')[0].replaceAll("-", "/");
	dataEvents[0].hora = dataEvents[0].date.toLocaleString("es-CL", {timeZone: 'America/Santiago'}).split(',')[1].slice(1, -3);	
